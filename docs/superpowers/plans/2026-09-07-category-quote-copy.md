...
## Environment constraint — read before starting

The development database listens on `localhost:55432` **on Vadym's machine**.
An agent working in the sandbox cannot reach it. That splits every task into
one of two kinds:

**Runnable by an agent:** editing files, `npm run typecheck`, `npm run lint`,
`npm test` (the whole suite is pure unit tests with no `DATABASE_URL` — see
`vitest.config.ts` and the header comments in `tests/quotation-data.test.ts`),
and `npx prisma generate` (reads the schema, never connects).

**Vadym runs, locally:** `npx prisma migrate dev`, `npm run db:migrate`,
`npm run db:seed`, `npx prisma db execute`, `npx tsx scripts/...`, and every
by-hand UI check against `npm run dev`.

So migrations are **hand-written** into `prisma/migrations/<timestamp>_<name>/migration.sql`
rather than generated by `prisma migrate dev`, and applied with
`npm run db:migrate` (`prisma migrate deploy`), which accepts a hand-written
migration and records its checksum on first apply. Each task below marks its
Vadym-only steps as **[LOCAL]**. An agent must not report a task complete on
the strength of a step it could not run — it reports what it ran, and lists
what is waiting on a local run.
...
Match the naming of the existing folders under `prisma/migrations/` (run
`ls prisma/migrations` and copy the timestamp format exactly). Create
`prisma/migrations/<timestamp>_series_quote_description/migration.sql`
containing only:

```sql
-- AlterTable
ALTER TABLE "Series" ADD COLUMN "quoteDescription" TEXT;
```

Nullable with no default, so applying it to a populated database rewrites no
rows and cannot fail.
...
Expected: "Generated Prisma Client". This reads the schema only and needs no
database, so `Series.quoteDescription` becomes visible to TypeScript
immediately.

- [ ] **Step 4: [LOCAL] Apply and verify**

Vadym runs, on his own machine:
...
Expected: the migration applies, then one row — `text`, `YES`.
...
- [ ] **Step 3: [LOCAL] Verify by hand**

Vadym runs `npm run dev`, then as an admin opens
`http://localhost:3100/catalog/<M-Series id>` and checks:
...
- [ ] **Step 2: [LOCAL] Verify by hand**

With the dev server running, Vadym puts `{{cutHeightCm}}` into the L-Series
category copy (L-Series products carry no cut height), opens a draft quote
containing an L-Series machine, and confirms the banner names `cutHeightCm`
and the line is absent from the sheet. Then finalizes and confirms the banner
is gone.
...
written. Read the report and confirm the M and X bodies are the expected ones.
...
Because the script cannot be run from the sandbox, it must be written
defensively: dry-run by default, refuse to overwrite non-empty copy, print
every intended write and delete before performing any, and be safe to run
twice.
...
`prisma/migrations/<timestamp>_drop_content_block_keys/migration.sql`:
...
Then `npx prisma generate`. Vadym applies it with `npm run db:migrate`
**[LOCAL]**. This is the one destructive migration in the plan: it drops
columns whose content Task 10 has already copied onto categories. Do not write
it until Task 10 has actually been applied against the database — an agent
cannot confirm that, so ask before committing this step.
...
- [ ] **Step 3: Save the drafts as a file**

The sandbox cannot write to the database, so put the five texts in
`RAW/category-quote-copy.md` — one heading per category, the HTML body under
it, and a note of which tokens it uses. Vadym pastes each into its category's
Quote description card.

- [ ] **Step 4: [LOCAL] Enter and check each**
...
cd "/Users/vadym/Documents/PF Invoice"
rm -f .git/index.lock .git/HEAD.lock
git add RAW/category-quote-copy.md
git commit -m "docs: draft quote copy for the five empty categories"
```
