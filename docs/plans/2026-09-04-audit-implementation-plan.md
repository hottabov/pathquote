...
| **`xlsx`** | фіксу немає | Prototype pollution + ReDoS у SheetJS. Пакет **імпортується рівно у трьох місцях**: `scripts/extract-catalog.ts:22`, `scripts/extract-us-prices.ts:63`, `tests/production-forms-contract.test.ts:4`. Це `devDependency`, він не потрапляє ні в застосунок, ні в образ, і працює на файлах, які ми самі кладемо в `RAW/`. Виробничі форми будує `src/lib/production-forms/xlsx-patch.ts` **на `fflate`**, не на цьому пакеті. |
